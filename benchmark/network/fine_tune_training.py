...
class RMSELoss(nn.Module):
    def __init__(self):
        ...
        self.mse = nn.MSELoss()  # 使用内置的 MSE 损失

    def forward(self, y_pred, y_true):
        return torch.sqrt(self.mse(y_pred, y_true))  # 取平方根
...
def main():
    ...
    for head in range(output_dim, atom_embedding_dim):
        if atom_embedding_dim % head == 0:
            num_heads = head
            break
    print('Actual heads number:', num_heads)
    ...
    criterion = RMSELoss() # 这是 RSME 损失函数
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            print(f"Saved best model with validation loss {best_val_loss:.8f} to {args.save_model}")
