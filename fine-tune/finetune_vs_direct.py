...
def main() -> None:
    ...
    set_nature_style()
    background = "white"
    plt.figure(figsize=(10, 6), facecolor=background)
    ...
    if s_d.size:
        plt.errorbar(
            ...
            fmt="s--",
            capsize=4,
            # label="Direct Train Model",
            color="#ABD3E1",
        )

    plt.xlabel("Number of training molecules", fontsize=24)
    plt.ylabel("Validation R²", fontsize=24)

    # 使用对数刻度但显式显示原始数值
    x_ticks = np.log10(all_sizes)
    plt.xticks(x_ticks, labels=[f"{x:,}" for x in all_sizes], fontsize=18)
