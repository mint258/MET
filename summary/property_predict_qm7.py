...
import matplotlib as mpl
# 依赖
from dataset_qm7 import MoleculeDataset
...
def plot_compare_nature(true_a, pred_a, true_b, pred_b,
                        ...
                        prop_name, save_path):
    # 2. 创建画布
    fig, ax = plt.subplots(figsize=(6,6))  # 方形画布，5×5英寸
    ...
    # 强制等比例显示（box 模式下，单位长度相同）
    ax.set_aspect('equal', adjustable='box')
    tick_vals = np.linspace(mn_pad, mx_pad, 5)
    lbs       = [str(int(round(v, -2))) for v in tick_vals]
    ...
    # 6. 坐标轴标签
    ax.set_xlabel(f'Real Energy', labelpad=4)
    ax.set_ylabel(f'Predicted Energy', labelpad=4)
